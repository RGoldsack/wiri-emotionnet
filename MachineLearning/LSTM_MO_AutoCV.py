...
def evaluate_model(X, y, y_cols, dataOptions, runtimeOptions):
    ...
    for train_ix, test_ix in cv.split(X):
        ...
        # loss prep
        cur_location = str(dataOptions[0]) + "_" + str(dataOptions[1]) + "_" + str(dataOptions[2]) + "_" + str(runtimeOptions[4]) + "_V" + str(runtimeOptions[5]) + "/"
        # print(cur_location)
...
#        pyplot.savefig(plot_name)
        ...
        path_y_PrAc = path + "Results/" + "y_PrAc" + "_" + str(cvNum) + "_" + str(dataOptions[0]) + "_" + str(dataOptions[1]) + "_" + str(dataOptions[2]) + "_" + str(runtimeOptions[4]) + "_V" + str(runtimeOptions[5]) + ".csv"
...
def LSTM_big(path, dataOptions, runtimeOptions, sectionList = None, random_order = False):
    ...
    pathRes = path + "Results/" + "Results" + "_" + str(dataOptions[0]) + "_" + str(dataOptions[1]) + "_" + str(dataOptions[2]) + "_" + str(runtimeOptions[4]) + "_V" + str(runtimeOptions[5]) + ".csv"
#   pathRes = path + "Results/" + "Results" + "_" + "section"           + "_" + "emotion"           + "_" + "phys"              + "_" + "random"               + "_V" + "valence"              + ".csv"
...
if getcwd() == "C:\\Users\\golds\\Downloads":
    ...
    n_epochs = {"phys": 250, "mocap": 100, "both": 100}

    dataOptions = [int(argv[1]),                   # section         [0]
                   str(argv[2]),                   # emotion         [1] - "6emo", "cont", "indiv.PANAS", "sum.PANAS"
                   str(argv[3])]                   # phys            [2] - "both", "phys", "mocap"
    
    runtimeOptions = [n_epochs[dataOptions[2]],    # epochs          [0]
                      2048,                        # batch size      [1]
                      2,                           # verbose         [2] - 0, 1, 2
                      True,                        # stateful        [3] - True, False
                      str(argv[4]),                # random          [4] - "shuf", "rand", "observed"
                      str(argv[5])]                # valence         [5] - "both", "pos", "neg"
