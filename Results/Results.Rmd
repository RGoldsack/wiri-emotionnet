...
$$
\begin{aligned}
\text{Overall Prediction Error}_{ijklmo} &= \sum_{x = 1}^{X} \text{Individual Emotion Error}_{ijklxo}, \\
...
\text{Where X = 1 }, \text{Overall Prediction Error}_{ijklmo} &= \text{Individual Emotion Error}_{ijklxo} \\\\
&\text{When ignoring the other levels of the Machine Learning Models,} \\
\text{Overall Prediction Error}_m &= \sum_{x = 1}^{X} \text{Individual Emotion Error}_{x} \\
\text{Where X = 1 }, \text{Overall Prediction Error}_m &= \text{Individual Emotion Error}_{x} 
\end{aligned}
$$
...
## Categorical & Continuous Cross-Entropy

1. Discrete Case

Consider a set of input-output pairs $\{(x^{(i)}, y^{(i)})\}_{i \in \mathcal{A}}$, where $x^{(i)} \in \mathbb{R}^N$ ($N$-dimensional vector of reals), $y^{(i)} \in \mathcal{B}$, where $\mathcal{B}$ is a set of discrete scalar values, and where $\mathcal{A}$ is a set of indices corresponding to the observations. We assume the input-output data pairs $(x^{(i)}, y^{(i)})$ are samples drawn directly from the ground-truth joint distribution of the random variables $X$ and $Y$. We are interested in the conditional distribution of $Y$ given $X$, which we write as $p_{Y|X}(y|x)$. As is usual, where this is not ambiguous we will simply write $p(y|x)$.

Note that $p(y|x)$ even if the map from $X$ to $Y$ is assumed to be deterministic (known value for $Y$ for known value of $X$), the map $p(y|x)$ is still meaningful. Then adn only then is $p(y|x)$ a one-hot vector for each $x$. This deterministic model ("the data are precisely right" is a standard assumption).

We aim to be approximate $p(y|x)$ with a parametric model, which we will denote as $q(y|x)$. We will implement $q(y|x)$ as a neural network $g(x)$ with as the final layer a softmax function. Almost never will $q(y|x)$ be one-hot. Our model is now a *deterministic function* that maps to inputs to probabilities $g(x)=q(y|x)$. For a paticular input value $x$ the network output is a vector of probabilities, one for each value in $\mathcal{B}$. Formally we can write $g$ as $g : \mathbb{R}^N \rightarrow [0, 1]^{|\mathcal{B}|}$ (map from $N$-dimensional vector of reals to a vector of numbers in $[0, 1]$ with $||g(x)||_1 = 1$ - their sum is 1).

The function $g$ has parameters $\theta$ that we can optimise. We can make that explicit and write $g(x; \theta)$ and $q(y|x; \theta)$. We want to find the best $\theta$. 

The most common (but certainly not only) measure that indicates how different two distributions is the Kullback-Leibler divergence. It is not perfect, for example it is not symmetric (hence divergence, not distance). Note that in our case we havea conditional distribution and one interpretation of our optimisation problem is that we simultaneously try to make all $q(y|x, \theta$
